 ...
 using namespace std;   
    ...
    void setPrm(vector<int> &v, string &ans, int n, int k, vector<int> factVal)
    {
        if(n==1) {ans += to_string(v.back()); return;}
        
        // calculate the starting number
        int index = k/factVal[n-1];
        if(k % factVal[n-1] == 0) index--; // corner case
        ans += to_string(v[index]);
        v.erase(v.begin()+index);
        k -= factVal[n-1]*index; // remainder
        setPrm(v,ans,n-1,k,factVal);
    }
